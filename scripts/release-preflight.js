...
import { isIgnorableReleaseDirtyPath, porcelainPaths } from './release-session.js';
...
function expectedReleaseBranch(version) {
  ...
  if (new RegExp(`^${numeric}\\.${numeric}\\.${numeric}-beta\\.${numeric}$`).test(version)) {
    return 'beta';
  }
  if (new RegExp(`^${numeric}\\.${numeric}\\.${numeric}-alpha\\.${numeric}$`).test(version)) {
    return 'beta';
  }
