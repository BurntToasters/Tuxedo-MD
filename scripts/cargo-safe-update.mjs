...
export function parseArguments(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument.startsWith('--allow-young=')) {
      allowYoung.push(argument.slice(14));
      continue;
    }
    if (argument.startsWith('--allow-git=')) {
      allowGit.push(argument.slice(12));
      continue;
    }
    if (argument === '--reason') {
      reason = argv[++index] ?? '';
...
function parsePackageVersion(value) {
  ...
  return { key: value, packageName: value.slice(0, separator), value: value.slice(separator + 1) };
}
...
function cargoSupportsTemporaryLockfile() {
  const match = run('cargo', ['--version']).stdout.match(/cargo\s+(\d+)\.(\d+)/i);
  if (!match) return false;
  return Number(match[1]) > 1 || (Number(match[1]) === 1 && Number(match[2]) >= 97);
}
...
function findWorkspaceRoot(manifest) {
  ...
  while (true) {
    ...
    if (existsSync(cargoToml) && /^\[workspace\]/m.test(readFileSync(cargoToml, 'utf8')))
      return directory;
    const parent = path.dirname(directory);
...
function copyWorkspace(sourceRoot, destinationRoot) {
  cpSync(sourceRoot, destinationRoot, {
    ...
    filter(source) {
      ...
      if (IGNORED_COPY_DIRECTORIES.has(relative.split(path.sep)[0])) return false;
      try {
...
async function main() {
  ...
  try {
    ...
    if (!validation.newlySelected.length) console.log('No new dependency versions selected');
    ...
    installValidatedLock(candidateLock, realLock ?? destination, parsed.cargoArgs, cwd);
