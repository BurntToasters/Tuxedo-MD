...
export function verifyUpdaterSignatures({
  ...
  if (missingSignatures.length > 0) {
    throw new Error(`Missing updater signature file(s): ${missingSignatures.sort().join(', ')}.`);
  }
  if (eligibleArtifacts.length === 0) {
    throw new Error('Updater signature verification found no updater-eligible artifacts.');
  }
  if (pairs.length === 0) {
    throw new Error('Updater signature verification found zero eligible pairs.');
  }
  ...
  const temporaryDirectory = fs.mkdtempSync(path.join(releaseDir, '.updater-verify-'));
  try {
    const publicKeyPath = path.join(temporaryDirectory, 'updater.pub');
    fs.writeFileSync(publicKeyPath, Buffer.from(encodedPublicKey, 'base64').toString('utf8'));
    const verifierArgs = [
      'run',
      '--quiet',
      '--manifest-path',
    ...
    for (const [index, [artifactPath, signaturePath]] of pairs.entries()) {
      const normalizedPath = path.join(temporaryDirectory, `signature-${index}.minisig`);
      fs.writeFileSync(
        normalizedPath,
        Buffer.from(normalizeUpdaterSignature(signaturePath), 'base64').toString('utf8')
      );
