...
function Get-TuxedoVisualStudioInstallations {
...
function Get-TuxedoCmakePathFromInstallation {
...
function Test-TuxedoCmakeListsGenerator {
  param(
...
function Resolve-TuxedoCmakeExecutable {
  $installs = Get-TuxedoVisualStudioInstallations
  ...
  foreach ($install in $installs) {
    [void]$candidates.Add((Get-TuxedoCmakePathFromInstallation -InstallationPath $install.Path))
  }
  ...
  foreach ($candidate in $candidates) {
    ...
    if ($needsVs18Generator -and
      -not (Test-TuxedoCmakeListsGenerator -CmakeExe $resolved -GeneratorName 'Visual Studio 18 2026')) {
...
function Resolve-TuxedoCmakeVsGenerator {
...
function Resolve-TuxedoVsDevShellLauncher {
  $vswhere = Join-Path ${env:ProgramFiles(x86)} 'Microsoft Visual Studio\Installer\vswhere.exe'
  if (Test-Path -LiteralPath $vswhere) {
  ...
  foreach ($install in (Get-TuxedoVisualStudioInstallations)) {
