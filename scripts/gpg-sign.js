...
const ARTIFACT_RULES = [
  ...
  rx(/\.(?:exe|msi|dmg|deb|rpm|appimage)\.sig$/i),
  rx(/^TuxedoMD(?:-macOS)?\.zip\.sig$/i),
  rx(/^Tuxedo\.MD_.+_universal\.app\.zip\.sig$/i),
  rx(/\.nsis\.zip\.sig$/i),
  rx(/\.tar\.gz\.sig$/i),
...
function rpmArtifactMatchesVersion(name, releaseVersion = VERSION) {
  ...
  const prereleaseMatch = betaMatch || alphaMatch;
  const numericVersion = prereleaseMatch?.[1] ?? stableMatch[1];
  ...
  const versionPattern = betaMatch
    ...
    : alphaMatch
      ? `${escapedNumericVersion}(?:-alpha\\.${alphaMatch[2]}|[._~]alpha[._-]${alphaMatch[2]})`
...
function walk(dir, results = []) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (/msstore$/i.test(entry.name)) continue;
      walk(full, results);
    } else if (
      entry.isFile() &&
      !isStoreOrProArtifact(entry.name, full) &&
      isArtifact(entry.name)
    ) {
      results.push(full);
    }
  }
...
function collectArtifacts() {
  ...
  for (const name of fs.readdirSync(releaseDir)) {
    ...
    if (!isFile || !wasBuiltInSession(fullPath, buildSession)) continue;
    const cleanName = cleanArtifactName(name);
    const holdPath = path.join(stagedHoldDir, cleanName);
    fs.copyFileSync(fullPath, holdPath);
    preservedStaged.push({ cleanName, holdPath, originalName: name });
  }
  ...
  if (found.length > 0) {
    ...
    for (const src of selected) {
      ...
      collected.push(dest);
      collectedNames.add(cleanName);
    }
    for (const staged of preservedStaged) {
      if (collectedNames.has(staged.cleanName)) continue;
      const dest = path.join(releaseDir, staged.cleanName);
      fs.copyFileSync(staged.holdPath, dest);
      console.log(
    ...
    fs.rmSync(stagedHoldDir, { recursive: true, force: true });
...
async function replaceReleaseAssetsTransactionally(
  ...
  try {
    ...
    for (const filePath of files) {
      ...
      const stagedName = `tuxedomd-pending-${token}-${name}`;
...
function isTransactionalStagingAssetName(name) {
  return /^(?:default\.)?tuxedomd-(?:pending|previous|rollback)-/i.test(name);
}
...
async function main() {
  console.log(`\nTuxedo MD ${VERSION}: release pipeline\n`);
  ...
  // Beta clients poll /releases/latest for latest-*-beta-*.json. Draft asset
